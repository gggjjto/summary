...
void LEDx_Init(uint32_t RCC_APB2Periph,uint16_t GPIO_Pin,GPIO_TypeDef* GPIOx)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph, ENABLE);             /* LED1时钟使能 */

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin;                         /* LED1引脚 */
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;                  /* 推挽输出 */
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;                 /* 高速 */
    GPIO_Init(GPIOx, &GPIO_InitStructure);                            /* 初始化LED1引脚 */
    
    GPIO_ResetBits(GPIOx, GPIO_Pin);                                  /* 关闭 LED1 */
}
